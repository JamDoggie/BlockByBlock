	...
	public class EntityGhast : EntityFlying, IMob
	{
		...
		private bool isCourseTraversable(double d1, double d3, double d5, double d7)
		{
			...
			for (int i16 = 1; (double)i16 < d7; ++i16)
			{
				...
				if (this.worldObj.getCollidingBoundingBoxes(this, axisAlignedBB15).Count > 0)
				{
					return false;
				}
			}
		...
		{
			int i3 = this.rand.Next(2) + this.rand.Next(1 + i2);
			...
			i3 = this.rand.Next(3) + this.rand.Next(1 + i2);
		...
		public override bool CanSpawnHere
		{
			get
			{
				return this.rand.Next(20) == 0 && base.CanSpawnHere && this.worldObj.difficultySetting > 0;
			}
		}
