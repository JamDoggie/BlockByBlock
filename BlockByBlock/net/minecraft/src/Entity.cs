...
namespace net.minecraft.src
{
	...
	public abstract class Entity
	{
		...
		protected internal virtual void preparePlayerToSpawn()
		{
			if (this.worldObj != null)
			{
				while (this.posY > 0.0D)
				{
					...
					if (this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox).Count == 0)
					{
						break;
					}
		...
		public virtual bool isOffsetPositionInLiquid(double d1, double d3, double d5)
		{
			...
			System.Collections.IList list8 = this.worldObj.getCollidingBoundingBoxes(this, axisAlignedBB7);
			return list8.Count > 0 ? false :!this.worldObj.isAnyLiquid(axisAlignedBB7);
		}
		...
		public virtual void moveEntity(double d1, double d3, double d5)
		{
			...
			else
			{
				...
				bool z18 = this.onGround && this.Sneaking && this is EntityPlayer;
				if (z18)
				{
					double d19;
					for (d19 = 0.05D; d1 != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.getOffsetBoundingBox(d1, -1.0D, 0.0D)).Count == 0; d11 = d1)
					{
						if (d1 < d19 && d1 >= -d19)
						{
							d1 = 0.0D;
					...
					for (; d5 != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.getOffsetBoundingBox(0.0D, -1.0D, d5)).Count == 0; d15 = d5)
					{
						if (d5 < d19 && d5 >= -d19)
						{
							d5 = 0.0D;
					...
					while (d1 != 0.0D && d5 != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.getOffsetBoundingBox(d1, -1.0D, d5)).Count == 0)
					{
						if (d1 < d19 && d1 >= -d19)
						{
							d1 = 0.0D;
				...
				System.Collections.IList list35 = this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.addCoord(d1, d3, d5));

				for (int i20 = 0; i20 < list35.Count; ++i20)
				{
					d3 = ((AxisAlignedBB)list35[i20]).calculateYOffset(this.boundingBox, d3);
				}
				...
				bool z36 = this.onGround || d13 != d3 && d13 < 0.0D;

				int i21;
				for (i21 = 0; i21 < list35.Count; ++i21)
				{
					d1 = ((AxisAlignedBB)list35[i21]).calculateXOffset(this.boundingBox, d1);
				}
				...
				for (i21 = 0; i21 < list35.Count; ++i21)
				{
					d5 = ((AxisAlignedBB)list35[i21]).calculateZOffset(this.boundingBox, d5);
				}
				...
				if (this.stepHeight > 0.0F && z36 && (z18 || this.ySize < 0.05F) && (d11 != d1 || d15 != d5))
				{
					...
					for (i28 = 0; i28 < list35.Count; ++i28)
					{
						d1 = ((AxisAlignedBB)list35[i28]).calculateXOffset(this.boundingBox, d1);
					}
					...
					for (i28 = 0; i28 < list35.Count; ++i28)
					{
						d5 = ((AxisAlignedBB)list35[i28]).calculateZOffset(this.boundingBox, d5);
					}
					...
					if (d37 * d37 + d25 * d25 >= d1 * d1 + d5 * d5)
					{
						...
						d3 = d23;
						d5 = d25;
						this.boundingBox.BB = axisAlignedBB27;
					}
					else
					{
		...
		{
			double d2 = this.posY + (double)this.EyeHeight;
			int i4 = MathHelper.floor_double(this.posX);
		...
		{
			double d3 = this.boundingBox.AverageEdgeLength;
		...
		{
			string string2 = this.EntityString;
			if (!this.isDead && !string.ReferenceEquals(string2, null))
			{
		...
		public virtual bool EntityInsideOpaqueBlock
		{
			get
			{
				for (int i1 = 0; i1 < 8; ++i1)
				{
					...
					int i6 = MathHelper.floor_double(this.posY + (double)this.EyeHeight + (double)f3);
		...
		public virtual void setPositionAndRotation2(double d1, double d3, double d5, float f7, float f8, int i9)
		{
			...
			this.setRotation(f7, f8);
			System.Collections.IList list10 = this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.contract(8.0D / 256D, 0.0D, 8.0D / 256D));
			if (list10.Count > 0)
			{
				double d11 = 0.0D;
		...
		public virtual int Air
		{
			...
			set
			{
				this.dataWatcher.updateObject(1, (short)value);
			}
		}
		...
		protected internal virtual bool pushOutOfBlocks(double d1, double d3, double d5)
		{
			...
			{
				bool z16 = !this.worldObj.isBlockNormalCube(i7 - 1, i8, i9);
				bool z17 = !this.worldObj.isBlockNormalCube(i7 + 1, i8, i9);
				bool z18 = !this.worldObj.isBlockNormalCube(i7, i8 - 1, i9);
				bool z19 = !this.worldObj.isBlockNormalCube(i7, i8 + 1, i9);
				bool z20 = !this.worldObj.isBlockNormalCube(i7, i8, i9 - 1);
				bool z21 = !this.worldObj.isBlockNormalCube(i7, i8, i9 + 1);
				sbyte b22 = -1;
				double d23 = 9999.0D;
				if (z16 && d10 < d23)
				{
