namespace net.minecraft.src
{
	public class EntityEnderman : EntityMob
	{
		...
		public override void writeEntityToNBT(NBTTagCompound nBTTagCompound1)
		{
			...
			nBTTagCompound1.setShort("carried", (short)this.Carried);
			nBTTagCompound1.setShort("carriedData", (short)this.CarryingData);
		}

		public override void readEntityFromNBT(NBTTagCompound nBTTagCompound1)
		{
			base.readEntityFromNBT(nBTTagCompound1);
			this.Carried = nBTTagCompound1.getShort("carried");
			this.CarryingData = nBTTagCompound1.getShort("carriedData");
		}
		...
		private bool shouldAttackPlayer(EntityPlayer entityPlayer1)
		{
			...
			else
			{
				...
				Vec3D vec3D4 = Vec3D.createVector(this.posX - entityPlayer1.posX, this.boundingBox.minY + (double)(this.height / 2.0F) - (entityPlayer1.posY + (double)entityPlayer1.EyeHeight), this.posZ - entityPlayer1.posZ);
		...
		public override void onLivingUpdate()
		{
			...
			if (!this.worldObj.isRemote)
			{
				...
				if (this.Carried == 0)
				{
					if (this.rand.Next(20) == 0)
					{
						...
						{
							this.Carried = this.worldObj.getBlockId(i1, i2, i3);
							this.CarryingData = this.worldObj.getBlockMetadata(i1, i2, i3);
				...
				else if (this.rand.Next(2000) == 0)
				{
					...
					{
						this.worldObj.setBlockAndMetadataWithNotify(i1, i2, i3, this.Carried, this.CarryingData);
						this.Carried = 0;
					}
				}
			}
			...
			if (!this.worldObj.isRemote && this.EntityAlive)
			{
				if (this.entityToAttack != null)
				{
		...
		protected internal virtual bool teleportRandomly()
		{
			...
			double d3 = this.posY + (double)(this.rand.Next(64) - 32);
		...
		{
			Vec3D vec3D2 = Vec3D.createVector(this.posX - entity1.posX, this.boundingBox.minY + (double)(this.height / 2.0F) - entity1.posY + (double)entity1.EyeHeight, this.posZ - entity1.posZ);
			vec3D2 = vec3D2.normalize();
			double d3 = 16.0D;
			double d5 = this.posX + (this.rand.NextDouble() - 0.5D) * 8.0D - vec3D2.xCoord * d3;
			double d7 = this.posY + (double)(this.rand.Next(16) - 8) - vec3D2.yCoord * d3;
			...
			if (this.worldObj.blockExists(i14, i15, i16))
			{
				...
				while (true)
				{
					...
					if (z17)
					{
						...
						if (this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox).Count == 0 && !this.worldObj.isAnyLiquid(this.boundingBox))
						{
							z13 = true;
						}
					}
		...
		public virtual int Carried
		{
			set
			{
				this.dataWatcher.updateObject(16, unchecked((sbyte)(value & 255)));
			}
			get
			{
		...
		public virtual int CarryingData
		{
			set
			{
				this.dataWatcher.updateObject(17, unchecked((sbyte)(value & 255)));
			}
			get
			{
