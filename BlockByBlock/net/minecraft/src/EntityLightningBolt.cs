	...
	public class EntityLightningBolt : EntityWeatherEffect
	{
		...
		public EntityLightningBolt(World world1, double d2, double d4, double d6) : base(world1)
		{
			...
			if (world1.difficultySetting >= 2 && world1.doChunksNearChunkExist(MathHelper.floor_double(d2), MathHelper.floor_double(d4), MathHelper.floor_double(d6), 10))
			{
				...
				for (i8 = 0; i8 < 4; ++i8)
				{
					i9 = MathHelper.floor_double(d2) + this.rand.Next(3) - 1;
					i10 = MathHelper.floor_double(d4) + this.rand.Next(3) - 1;
					int i11 = MathHelper.floor_double(d6) + this.rand.Next(3) - 1;
		...
		public override void onUpdate()
		{
			...
			{
				double d6 = 3.0D;
				System.Collections.IList list7 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, AxisAlignedBB.getBoundingBoxFromPool(this.posX - d6, this.posY - d6, this.posZ - d6, this.posX + d6, this.posY + 6.0D + d6, this.posZ + d6));
