using System;
using System.Collections;

namespace net.minecraft.src
{

	public class ComponentVillageStartPiece : ComponentVillageWell
	{
		public WorldChunkManager worldChunkMngr;
		public int terrainType;
		public StructureVillagePieceWeight structVillagePieceWeight;
		public ArrayList structureVillageWeightedPieceList;
		public ArrayList field_35108_e = new ArrayList();
		public ArrayList field_35106_f = new ArrayList();

		public ComponentVillageStartPiece(WorldChunkManager worldChunkManager1, int i2, Random random3, int i4, int i5, ArrayList arrayList6, int i7) : base(0, random3, i4, i5)
		{
			this.worldChunkMngr = worldChunkManager1;
			this.structureVillageWeightedPieceList = arrayList6;
			this.terrainType = i7;
		}

		public virtual WorldChunkManager WorldChunkManager
		{
			get
			{
				return this.worldChunkMngr;
			}
		}
	}

}